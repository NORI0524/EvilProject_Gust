Shader "Toon/Lit Dissolve"
{
    Properties
    {
        ...
        _NoiseTex("Dissolve Noise", 2D) = "white"{}  // ディゾルブ用のベーステクスチャ
        _NScale("Noise Scale", Range(0, 10)) = 1  // ディゾルブラインの色
        _DisAmount("Dissolve Amount", Range(0, 1)) = 0    // ディゾルブの強さ
        
        _DisLineWidth("Dissolve Width", Range(0, 2)) = 0  // ディゾルブ周りのラインの幅
        _DisLineColor("Dissolve Color", Color) = (1,1,1,1)   // ラインの色
        _DisLineColorEx("Dissolve Color Extra", Color) = (1,1,1,1) // ラインの色
    ...
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutput o)
        {
            ...
            half4 n = tex2D(_NoiseTex, IN.worldPos.xy * _NScale); // ノイズテクスチャを比較可能な値に変える。 worldPos.xyは片側からプロジェクト、xzは反対側から、yzは上から
            #if LIGHTMAP
